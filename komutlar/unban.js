...
exports.run = async (client, message, args) => {
  ...
  if (!message.member.roles.cache.has("759441120077152296")) return message.channel.send(`**Bu komutu kullanabilmek için "\`Üyeleri Yasakla\`" yetkisine sahip olmalısın.**`);
 ...
 if (isNaN(user)) return message.channel.send('**Lütfen Banını Açmak İstediğiniz Üyeninin ID sini Girin**');
  ...
  client.channels.cache.get('763430424978128916').send(embed)///LOG KANAL İD YAZMALISIN
