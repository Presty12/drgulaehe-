...
exports.run = async(client, message, args) => {
   if(!message.member.roles.cache.has('760404936592326666')) return message.channel.send('Bu komutu kullanabilmek için gerekli yetkiye sahip değilsin : `rôl adı`')
   let member = message.mentions.users.first() || client.users.cache.get(args.join(' '))
   if(!member) {
       return message.channel.send('Bir kişi etiketlemelisin')
   }
   let vip = message.guild.roles.cache.find(r => r.id === '759433911645372437')//Viprolİd Koy
  ...
  client.channels.cache.get('767355990261366814').send(embed)///LOG KANAL İD YAZMALISIN
}
